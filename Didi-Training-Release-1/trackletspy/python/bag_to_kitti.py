...
def process_rtk_data(
    ...
    tracklets = []
    cap_rear_gps_df = init_df(cap_data['gps'], GPS_COLS, 'cap_rear_gps.csv', outdir)
    cap_front_gps_df = init_df(cap_data['gps'], GPS_COLS, 'cap_front_gps.csv', outdir)
    cap_rear_rtk_df = init_df(cap_data['rtk'], POS_COLS, 'cap_rear_rtk.csv', outdir)
    cap_front_rtk_df = init_df(cap_data['rtk'], POS_COLS, 'cap_front_rtk.csv', outdir)
	...
	print(obs_rtk_dict.items())
        obs_front_rtk_df = init_df(obs_rtk_dict['rtk'], POS_COLS, '%s_front_rtk.csv' % obs_name, outdir)
        obs_rear_rtk_df = init_df(obs_rtk_dict['rtk'], POS_COLS, '%s_rear_rtk.csv' % obs_name, outdir)
        ...
        if len(obs_front_rtk_df.index):
            obs_rtk_dfs[obs_name]['front_rtk'] = obs_front_rtk_df
        if correct > 0:
    ...
    if correct == CORRECT_PLANE:
        ...
        for obs_name in obs_rtk_dfs.keys():
            if 'front' in obs_rtk_dfs[obs_name]:
                obs_rtk_dfs[obs_name]['obs_rtk'].loc[:, ['tx', 'ty', 'tz']] = corrected_points[pts_idx]
                pts_idx += 1
            obs_rtk_dfs[obs_name]['obs_rtk'].loc[:, ['tx', 'ty', 'tz']] = corrected_points[pts_idx]
            pts_idx += 1
...
def main():
    ...
    OBS_RTK_NAMES = ['obs1']
    OBS_FRONT_RTK_TOPICS = ['/' + x + '/gps/rtkfix' for x in OBS_RTK_NAMES]
    OBS_REAR_RTK_TOPICS = ['/' + x + '/gps/rtkfix' for x in OBS_RTK_NAMES]
    obstacle_topics += OBS_FRONT_RTK_TOPICS
    obstacle_topics += OBS_REAR_RTK_TOPICS
    print(OBS_FRONT_RTK_TOPICS)
    ...
    for bs in bagsets:
		...
		print(topic)
                pose2dict(timestamp, msg.pose, cap_data['rtk'])
                stats['msg_count'] += 1
		...
		print(topic)
                gps2dict(timestamp, msg, cap_data['gps'])
                stats['msg_count'] += 1
		...
		print(topic)
                ...
                pose2dict(timestamp, msg.pose, obs_data['obs1']['rtk'])
                stats['msg_count'] += 1
        ...
        if len(camera_df['timestamp']):
            ...
            if 'rtk' in cap_data and 'rtk' in cap_data:
