...
os.system("mkdir "+Dir+"tracklets")
print Dirs
for dirr in Dirs:
	print dirr
        Labels = [f for f in os.listdir(Dir+dirr) ]
	Labels.sort()
	with open(Dir+"/"+dirr+"/"+Labels[0]) as myfile:
	...
	for lab in Labels[1:]:
	    with open(Dir+"/"+dirr+"/"+lab) as myfile:
	        line = myfile.readline()
            if len(line.split())!=0:
                Infos = line.split()
            else:
	...
	Text = Text + "</boost_serialization>\n"
        ...
        with open(Dir+"tracklets/"+dirr+".xml", "w") as myfile:
            myfile.write(Text)
