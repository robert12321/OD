...
class KittiViewer(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.w_plot = QPushButton('START/STOP')
...
if __name__ == '__main__':
    ...
    ex = KittiViewer()
    ex.current_idx = 0
    #ex.on_loadDetPressed()
    ex.on_loadButtonPressed()
    #ex.on_plotButtonPressed()
    
    i=0
    #timer = QtCore.QTimer()
    
    def num():
        global i, timer, Stop, PREVV
        if Stop==False: # and i <5200
            #print ( i )
            if PREVV:
                di = -1
            else:
                di = 1
            ex.current_idx = ex.current_idx + di
            ex.on_nextOrPrevPressed(PREVV)
        else:
            timer.stop()
